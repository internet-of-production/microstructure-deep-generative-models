...
CONFIG = {
    ...
    "get_data_loaders": {
        ...
        "ngpu" : 1,
        "nc" : 1,
        "modelname":"GAN_Model_200_Manuscript",
        "resultdir":"dummytobeoverwritten",
        "resultpath": "dummytobeoverwritten",
        "batch_size": 20, #24--> Mode Collapse
        "augmentation": "on", #128
    ...
    "Discriminator": {
        ...
        "ndf" : 12, #12 f--> Mode collapse
        "device" : "cuda", #otherwise cpu
         #Learning Rate
        "lr" : 0.00052,
        'gamma':0.985 #0.99, 0.98 , 096G konvergiert nicht
    },
    "Generator": {
        # Size of feature maps in generator
        "ngf" :32, #54 --> mode collapse ,62 direkt vorbei , 44 mode collapse
        # Size of z latent vector (i.e. size of generator input)
        "nz" : 60, # for v4
        #Size of embedding
        'embed_size':20,#20 for v4
        #Learning Rate
        "lr" : 0.00055,
        'gamma':0.99 #vorher 0.99, klappt bis 100 epochen gut
    },
    "Training": {
        "classify":"on",
        "num_epochs": 1000,
...
if CONFIG["Training"]["classify"]=="on":
    ...
    #{str(CONFIG["get_data_loaders"]["scale_labels"]*int(k)):v for k,v in Label_Dic.items()}
...
if __name__ == "__main__":
    ...
    #CONFIG=read_modeldata(CONFIG)
    # Create the generator
    #netG = Generator(CONFIG,len(dataloader.dataset.classes)*scale,(len(dataloader.dataset.classes)*scale)**2).to(CONFIG["Discriminator"]["device"])
